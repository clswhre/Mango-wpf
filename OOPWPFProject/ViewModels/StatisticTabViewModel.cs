...
internal class StatisticTabViewModel : BaseViewModel
{
    ...
    public double? AverageRating
    {
        get
        {
            var validRatings = _store.Places.Where(p => p.Rating.HasValue).Select(p => p.Rating!.Value).ToList();
    ...
    public DateOnly? MostRecentVisitDate => _store.Places
        .Where(p => p.Date.HasValue)
        .OrderByDescending(p => p.Date)
        .FirstOrDefault()?.Date;

    public int VisitedPerCurrentYear => _store.Places
        .Count(p => p.IsVisited && p.Date.HasValue && p.Date.Value.Year == DateTime.Today.Year);
    ...
    public string MostVisitedCountry => _store.Places
        .Where(p => p.IsVisited)
        .GroupBy(p => p.Country)
        .OrderByDescending(g => g.Count())
        .FirstOrDefault()?.Key ?? "Не обраховано";
    ...
    {
        if (e.PropertyName == nameof(Place.Rating) ||
...
//    public double? AverageRating => Places.Any() ? Places.Average( p => p.Rating ) : null;

//    public int VisitedCount => Places.Count(p => p.IsVisited);

//    public DateOnly? MostRecentVisitDate => Places.Where(p => p.Date.HasValue).OrderByDescending(p => p.Date).FirstOrDefault()?.Date;

//    private int GetVisitedPerCurrentYear(List<Place> places) {
//        int currentYear = DateTime.UtcNow.Year;
//        int count = places.Count(p => p.Date.HasValue && p.Date.Value.Year == currentYear);
//        return count;
//    }

//    public int HistoricalPlacesCount => Places.Count(p => p is HistoricalPlace);
//    public int NaturalPlacesCount => Places.Count(p => p is NaturalPlace);

//    public string MostVisitedCountry => Places.Where(p => p.IsVisited).GroupBy(p => p.Country).OrderByDescending(g => g.Count()).FirstOrDefault()?.Key ?? "Не обраховано";
//}
