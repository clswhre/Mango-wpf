...
internal class ReviewsTabViewModel : BaseViewModel
{
	...
	public ReviewsTabViewModel(PlaceStore store)
	{
		...
		RemoveReviewCommand = new RelayCommand(
			...
			p => _selectedPlace != null && p is string
		);
	}
	...
	private bool CanAddReview() =>
		_selectedPlace != null && !string.IsNullOrWhiteSpace(NewReviewText);
	...
	private void AddReview()
	{
		...
		Logger.Log(
			LogLevel.Info,
			$"Дія (Змінено): Додано відгук для місця '{_selectedPlace.Name}'"
		);
	...
	{
		if (_selectedPlace == null || string.IsNullOrEmpty(reviewText))
			return;
		...
		Logger.Log(
			LogLevel.Info,
			$"Дія (Змінено): Видалено відгук для місця '{_selectedPlace.Name}'"
		);
		OnPropertyChanged(nameof(Reviews));
		AddReviewCommand.RaiseCanExecuteChanged();
		RemoveReviewCommand.RaiseCanExecuteChanged();
