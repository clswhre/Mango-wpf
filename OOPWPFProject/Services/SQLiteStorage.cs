...
public class SQLiteStorage
{
    ...
    public async Task<List<Place>> LoadAsync()
    {
        ...
        while (await reader.ReadAsync())
        {
            dtos.Add(new PlaceDto
            {
                ...
                Type = Enum.TryParse<PlaceType>(reader.GetString(1), out PlaceType type) ? type : PlaceType.Normal,
    ...
    private static void BindDto(SqliteCommand cmd, PlaceDto dto, bool isIdIncluded)
    {
        ...
        cmd.Parameters.AddWithValue("@ProtectedStatus", dto.ProtectedStatus.HasValue ? (dto.ProtectedStatus.Value ? 1 : 0) : DBNull.Value);
        ...
        cmd.Parameters.AddWithValue("@WeatherIconPath", dto.WeatherIconPath ?? (object)DBNull.Value);
    }
}
